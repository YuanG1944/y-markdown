...
use gpui_component::{AxisExt, PixelsExt, scroll::ScrollbarHandle};
...
impl VirtualList {
    pub fn new<V, R>(
        ...
        id: impl Into<ElementId>,
        axis: Axis,
        scroll_handle: VirtualListScrollHandle, // <--- 关键：接收外部传入的 handle
        item_sizes: Rc<Vec<Size<Pixels>>>,
        render_fn: impl 'static + Fn(&mut V, Range<usize>, &mut Window, &mut Context<V>) -> Vec<R>,
    ) -> Self
    where
        V: Render,
        R: IntoElement,
    {
        ...
        // 这个适配器闭包用于满足 VirtualList 内部对 render_items 的类型要求。
        // 它负责在渲染时重新获取视图上下文 (view.update)。
        let render_adapter = move |visible_range: Range<usize>,
                                   window: &mut Window,
                                   cx: &mut App|
              -> SmallVec<[gpui::AnyElement; 64]> {
            view.update(cx, |this, cx| {
                render_fn(this, visible_range, window, cx)
                    .into_iter()
        ...
        Self {
            ...
            base: div()
                .id(id)
                // 【重要布局修复】
                // 使用 flex_1() 让列表在父 Flex 容器中自动填充剩余空间。
                // 如果用 size_full()，它可能会撑破父容器导致滚动条不出现。
                .flex_1()
                .overflow_scroll()
                // 绑定传入的 handle
                .track_scroll(&scroll_handle),
            ...
            items_count: item_sizes.len(),
            item_sizes,
            render_items: Box::new(render_adapter),
            sizing_behavior: ListSizingBehavior::default(),
        }
    }
